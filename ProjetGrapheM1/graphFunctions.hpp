...
std::vector<std::pair<int, std::pair<int, int>>> rouletteRusseNodeMove(NodeBend& n, GridLayout& GL, ConstCombinatorialEmbedding& ccem, double& sommeLong, double& sommeLong2, double& variance, int gridHeight, int gridWidth) {
	...
	//pas de mouvement -> déplacement 8
...
int startBest(GridLayout& GL, ConstCombinatorialEmbedding& ccem, double& sommeLong, double& sommeLong2, double& variance, int gridHeight, int gridWidth) {
	...
	if (bestDeplacement.size() > 0) {
		...
		changeVariance(nb, GL, bestDeplacement[j].second.first, bestDeplacement[j].second.second, sommeLong, sommeLong2, variance);
		(*nb.a_x) = bestDeplacement[j].second.first;
		(*nb.a_y) = bestDeplacement[j].second.second;
	}
	return randomNum;
	std::cout << "Nouvelle variance " << variance << std::endl;
}
